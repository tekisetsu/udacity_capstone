...
from dbw_mkz_msgs.msg import ThrottleCmd, SteeringCmd, BrakeCmd, SteeringReport, TwistCmd
...
class DBWNode(object):
    def __init__(self):
        ...
        # dbw controller, it contains the yaw and throttle/acceleration controllers
        self.controller = Controller(vehicle_mass, brake_deadband, decel_limit, accel_limit, wheel_radius,
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if None not in [self.current_velocity, self.angular_velocity, self.linear_velocity]:
                self.throttle, self.brake, self.steer = self.controller.control(
                    self.dbw_enabled, self.linear_velocity, self.angular_velocity, self.current_velocity)

            # We publish the control commands if dbw is enabled
            if self.dbw_enabled:
